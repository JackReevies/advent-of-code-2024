module.exports.convert = function convert(html) {
  const articleRegex = /<article class="day-desc"><h2>--- (Day \d+: .+?) ---<\/h2><p>(.+?)<\/article>(?:.+?<article class="day-desc"><h2 id="part2">--- Part Two ---<\/h2><p>(.+?)<\/article>)?/gms.exec(html)
  const title = articleRegex[1]
  const content = articleRegex[2]
  const p2Content = articleRegex[3] || 'Not unlocked yet - submit part 1 and re-run node index download [day]'
